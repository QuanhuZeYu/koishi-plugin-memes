...
__export(src_exports, {
  Config: () => Config,
  QHZY_MEME_BASEDIR: () => QHZY_MEME_BASEDIR,
  apply: () => apply,
  name: () => name
});
...
// src/tools/index.ts
var tools = { urlToBuffer: getAvaImage_default };
var tools_default = tools;

// src/commands/petpet.ts
var import_memelib = require("@quanhuzeyu/memelib");
...
async function urlToPet(url) {
  const buf = await tools_default.urlToBuffer(url);
  const _b = import_memelib.MemeGenerator.tools.imageTools.isGif(buf);
  if (_b === false) {
    try {
      const cir = await import_memelib.MemeGenerator.tools.imageTools.cropToCircle(buf);
      const pet = await import_memelib.MemeGenerator.Petpet(cir);
    ...
    const pet = await import_memelib.MemeGenerator.Petpet(buf, true);
    if (pet instanceof Buffer) {
      return import_koishi.h.image(pet, "image/gif");
    } else {
      return "制作头像时出现错误";
...
// src/index.ts
var name = "模因";
var QHZY_MEME_BASEDIR = __dirname;
